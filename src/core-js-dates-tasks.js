...
function getNextFriday(date) {
  ...
  const newData = new Date(date.setDate(date.getDate() + diff));
  return newData;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const MINSECK = 24 * 60 * 60 * 1000;
  const count = Math.round((newDateEnd - newDateStart) / MINSECK + 1);
  return count;
}
...
function formatDate(date) {
  ...
  const ampm = data.getUTCHours() >= 12 ? 'PM' : 'AM';
  return `${data.getUTCMonth() + 1}/${data.getUTCDate()}/${data.getUTCFullYear()}, ${data.getUTCHours() % 12 || 12}:${String(data.getUTCMinutes()).padStart(2, '0')}:${String(data.getUTCSeconds()).padStart(2, '0')} ${ampm}`;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (current <= end) {
    for (let i = 0; i < countWorkDays && current <= end; i += 1) {
      result.push(formatDate8(current));
      current.setDate(current.getDate() + 1);
    }
    for (let j = 0; j < countOffDays && current <= end; j += 1) {
      current.setDate(current.getDate() + 1);
    }
  }
